class Note{
    constructor(title,content,pinned,color){
        this.title = title;
...
let openWeatherUrl = `https://api.openweathermap.org/data/2.5/weather?q=Krakow&APPID=${api.key}`;
function getResults(){
    fetch("https://api.openweathermap.org/data/2.5/weather?q=Krakow&APPID=334f64f5734edf8ec8c3143c2844165b")
        ...
        .then(function(data){
            ...
            title= data.name;
            window.contents = data.main.temp;
            window.color="red";
        })
        .then(displayWeather);
}
...
function displayWeather(){
    const n =  new Note(title,window.contents,true,window.color);
    notes.addNote(n);
}
